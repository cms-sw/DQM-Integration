...
def trigvalegammaZ(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT Egamma/Zee Preselection/" + p] = DQMItem(layout=rows)
...
def trigvalegammaW(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT Egamma/Wenu Preselection/" + p] = DQMItem(layout=rows)
...
def trigvalmuon(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT Muon/" + p] = DQMItem(layout=rows)
...
def trigvaltau(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT Tau/" + p] = DQMItem(layout=rows)
...
def trigvaljetmet(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT JetMET/" + p] = DQMItem(layout=rows)

###---- BJET selection goes here: ----
def trigvalbjet(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT BJet/" + p] = DQMItem(layout=rows)

###---- ALCA selection goes here: ----

def trigvalalca(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT AlCa/" + p] = DQMItem(layout=rows)
...
def trigvalbphys(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT HeavyFlavor/" + p] = DQMItem(layout=rows)
...
def trigvaltop(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT Top/" + p] = DQMItem(layout=rows)
...
def trigvalsusybsm(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT SusyExo/" + p] = DQMItem(layout=rows)
...
def trigvalhiggs(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT Higgs/" + p] = DQMItem(layout=rows)
#trigvalhiggs(dqmitems,"", [{'path': "HLT/", 'description': ". For more information please click <a href=\"\">here</a>."}])


###---- QCD selection goes here: ----
def trigvalqcd(i, p, *rows): i["00 Shift/TRIGGER Validation/HLT QCD/" + p] = DQMItem(layout=rows)
