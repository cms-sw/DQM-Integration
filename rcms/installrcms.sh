#!/bin/bash

ln -s  daq_dqm_sourceclient-live.cfg           dqmfu10-1.cfg 
ln -s  sistrip_dqm_sourceclient-live.cfg       dqmfu15-1.cfg 
ln -s  pixel_dqm_sourceclient-live.cfg         dqmfu15-2.cfg 
ln -s  shipper_dqm_sourceclient-live.cfg       dqmfu15-3.cfg 
ln -s  hlx_dqm_sourceclient-live.cfg           dqmfu15-4.cfg 
ln -s  ecal_dqm_sourceclient-live.cfg          dqmfu16-1.cfg 
ln -s  hcal_dqm_sourceclient-live.cfg          dqmfu16-2.cfg 
ln -s  dt_dqm_sourceclient-live.cfg            dqmfu16-3.cfg 
ln -s  hlt_dqm_sourceclient-live.cfg           dqmfu16-4.cfg 
ln -s  csc_dqm_sourceclient-live.cfg           dqmfu17-1.cfg 
ln -s  l1t_dqm_sourceclient-live.cfg           dqmfu17-2.cfg 
ln -s  rpc_dqm_sourceclient-live.cfg           dqmfu17-3.cfg 
ln -s  l1temulator_dqm_sourceclient-live.cfg   dqmfu17-4.cfg 

