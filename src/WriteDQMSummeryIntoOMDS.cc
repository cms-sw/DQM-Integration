...
WriteDQMSummeryIntoOMDS::WriteDQMSummeryIntoOMDS(
...
void
WriteDQMSummeryIntoOMDS::run()
{
}

void WriteDQMSummeryIntoOMDS::dropTable(const std::string & table){
  m_tableToDrop = table;
...
void WriteDQMSummeryIntoOMDS::dropView(const std::string & view){
  m_viewToDrop = view;
  coral::ISession* session = this->connect( m_connectionString,m_user, m_pass );
  session->transaction().start( );
  std::cout << "connected succesfully to omds" <<std::endl;
...
std::vector<Item>
WriteDQMSummeryIntoOMDS::readData(const std::string & file)
{
...
void
WriteDQMSummeryIntoOMDS::writeData(const std::string & tableToRead)
  {
