...
process.add_(
    ...
    outputFileName = cms.untracked.string('%s/iSpy_%s%d_%s__hltOutputExpress_.ig' % (process.dqmSaver.dirName.value(),BaseName,int(dt.date.today().strftime("%W"))+1,dt.date.today().strftime("%Y%m%d"))),
